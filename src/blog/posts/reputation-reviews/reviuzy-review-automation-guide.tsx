...
export const meta: BlogPostMeta = {
  ...
  faqItems: [
    {
      question: 'How does the AiLys Reviuzy add-on tie into review velocity?',
      answer:
        'Reviuzy raises Google review velocity by automating the parts that owners forget on a busy week. NFC tap collection at the front desk, AI assisted draft suggestions inside the customer flow, auto-replies on every new review, and a monthly contest engine that the client runs to push spikes when needed. The AiLys engine then reads the new reviews into the GBP optimization layer so the keyword variety and recency signals lift the local pack score in the same week.',
    },
    {
      question: 'How much does Reviuzy cost and how do I add it to my AiLys plan?',
      answer:
        'Reviuzy is 100 dollars CAD per month as a standalone add-on. It is bundled at no extra cost in the AiLys Agency tier at 2,500 dollars per month. Owners on Starter, Core, or Growth can add Reviuzy on the pricing page or ask the strategist to attach it to the next billing cycle. The add-on includes the NFC stickers, the dashboard, the auto-reply workflow, the contest engine, and the legal template library.',
    },
    {
      question: 'Does Reviuzy run my contest for me or do I run it?',
      answer:
        'The client runs the contest. Reviuzy provides the setup, the legal terms and conditions generator, the help docs, and the dashboard for picking the winner. The client owns the prize, the customer relationship, and any tax or regulatory disclosure that applies in their jurisdiction. AiLys does not draw the winner and does not communicate with contestants. This separation keeps the agency on the right side of the contest rules in Quebec and Canada.',
    },
    {
      question: 'Who uploads the photos when a contest winner is announced?',
      answer:
        'The client uploads the photos through the Reviuzy app. AiLys does not source winner photos and does not upload to GBP on behalf of the contest. The Reviuzy app provides the upload form, the consent capture, and the GBP push, but the human action stays with the operator. This protects the client from copyright issues and keeps the photo provenance clean for Google.',
    },
    {
      question: 'How does Reviuzy detect fake reviews and what does it do about them?',
      answer:
        'The Reviuzy fake review detector scores every new review on a few signals: account age, review history pattern, language model authorship probability, and IP cluster proximity. Reviews flagged above a threshold are surfaced in the dashboard with a one-click flag-to-Google action. The system does not auto-flag without owner approval, because Google penalizes false positives. The owner decides which flags to push to Google.',
    },
    {
      question: 'Are auto-replies safe or will Google penalize templated responses?',
      answer:
        'Auto-replies are safe when they are personalized per review. Reviuzy generates a draft tailored to the review text, the reviewer name, and the service mentioned. The owner approves or edits the draft inside one click, and the reply ships to GBP through the official Google API. Templated responses that are identical across reviews trigger pattern detection and can soft-suppress the listing. The Reviuzy convention is one approved draft per review, not a copy-paste pattern.',
    },
  ],
  relatedSlugs: ['google-review-velocity-playbook', 'ailys-vs-traditional-seo-agency'],
  headings: [
    { id: 'what-reviuzy-is-and-what-it-is-not', text: 'What Reviuzy is and what it is not' },
    ...
    { id: 'how-reviuzy-wires-into-gbp-and-the-ailys-engine', text: 'How Reviuzy wires into GBP and the AiLys engine' },
    { id: 'when-reviuzy-pays-for-itself', text: 'When Reviuzy pays for itself' },
...
export function Content() {
  return (
    <article>
      ...
      <h2 id="what-reviuzy-is-and-what-it-is-not">What Reviuzy is and what it is not</h2>
      <p>
        Reviuzy is a reputation automation product. It collects reviews, drafts replies, monitors fakes, and pushes the data into the AiLys engine that reads recency and keyword variety as ranking inputs. It runs as a sister product to the AiLys local SEO platform, with a separate dashboard for the operator and a unified billing inside the AiLys account.
      </p>
      <p>
        Reviuzy is not a content production service. It does not write blog posts. It does not run the operator's social media. It does not source photography for the Google Business Profile, because the photos are uploaded by the client through the Reviuzy app to keep the provenance clean. Reviuzy is also not a contest operator. The client runs the contest, and Reviuzy provides the setup, the legal templates, and the help docs.
      </p>
      <p>
        The honest framing is that Reviuzy is a velocity engine for review collection and reply, with an attached toolkit for contest campaigns and fake review handling. Owners who treat it as that get the lift. Owners who expect it to replace a marketing manager will find it underdelivers, because the human approval step is intentional and stays with the operator.
      </p>
      ...
      <InlineCTA variant="pricing" text="See the AiLys plans and where Reviuzy is bundled or added on top, from Starter at 300 dollars to Agency at 2,500 dollars CAD." />

      <SectionDivider />

      <h2 id="nfc-tap-to-review-the-front-desk-loop">NFC tap-to-review, the front desk loop</h2>
      <p>
        The NFC tap-to-review sticker sits on the counter or the receipt holder. The customer taps a phone, the device opens a Reviuzy landing page, the page surfaces the direct Google review link with a one-tap flow into the GBP review form. The whole experience runs under five seconds from tap to review composition. Stickers ship as part of the Reviuzy onboarding kit at no extra cost.
      </p>
      <p>
        The reason the NFC loop works is that it removes the two friction points that kill review velocity. First, the customer does not have to find the GBP listing on Google, because the Reviuzy landing page jumps straight to the review form. Second, the customer does not have to remember to leave the review later, because the prompt happens at the moment of satisfaction. Most operators see review velocity double inside the first 60 days of NFC deployment.
      </p>
      <p>
        The Reviuzy landing page also gates a quick service question that filters the experience without violating Google review policy. The honest version routes every customer to the Google review link, and uses the gate only to decide which review template to suggest, not to filter out unhappy customers. Filtering by satisfaction breaks Google policy and can soft-suppress the listing.
      </p>
      ...
      <ul>
        ...
        <li>The Reviuzy landing page configured with the GBP review link</li>
      ...
      <CalloutBox type="warning">
        <p>The AI draft is a starting point, not a script. If the customer copies the suggestion verbatim and submits, the review reads as templated and Google can pattern-detect it. The Reviuzy UI nudges the customer to personalize the suggestion before submitting, with a small visual cue when the draft has not been edited. This nudge improves review quality and protects the listing.</p>
      </CalloutBox>
      ...
      <QuickQuiz
        question="What is the right convention for Reviuzy auto-replies on Google reviews?"
        options={[
      ...
      <p>
        Photo handling for contest winners follows the same separation. The client uploads the winner photo through the Reviuzy app, the app captures consent, and the photo pushes to GBP through the official API. AiLys does not source winner photos and does not upload on behalf of the contest. This protects the client from copyright issues and keeps the photo provenance clean for Google.
      </p>
      ...
      <p>
        Every new review runs through the Reviuzy fake detection scorer on arrival. The scorer reads a few signals: account age, review history pattern, language model authorship probability, IP cluster proximity, and timing patterns relative to the business operating hours. Reviews above the flag threshold are surfaced in the dashboard with a one-click flag-to-Google action.
      </p>
      <p>
        The system does not auto-flag without owner approval, because false positives carry a penalty. Google has tightened the review reporting rules and now soft-suppresses listings that flag legitimate reviews repeatedly. The Reviuzy convention is one human decision per flag, with the AI providing the score and the suggested rationale text.
      </p>
      <p>
      ...
      <h2 id="how-reviuzy-wires-into-gbp-and-the-ailys-engine">How Reviuzy wires into GBP and the AiLys engine</h2>
      <p>
        Reviuzy connects to Google Business Profile through the official Google API with read and write scopes for reviews, replies, and (with owner consent) photo uploads. The connection is OAuth-based, the token is stored encrypted, and the operator can revoke access at any time from the dashboard. No credentials live outside the Reviuzy vault.
      </p>
      <p>
      ...
      <h2 id="when-reviuzy-pays-for-itself">When Reviuzy pays for itself</h2>
      <p>
      ...
      <p>
        The owners who get the most lift are the ones who treat Reviuzy as a daily habit. The dashboard becomes the morning email check and the post-lunch reply pass. The strategist on the AiLys side tracks the velocity in the weekly report and adjusts the GBP optimization layer when the review keyword mix shifts. The combined effect is a local pack score that climbs steadily across one quarter.
      </p>
      <p>
        The owners who get the least lift are the ones who buy Reviuzy and forget about it. The dashboard goes unread, the auto-replies stack up unsent, and the contest engine never launches. In that case the spend is wasted, and the honest recommendation is to cancel the add-on and revisit it after the operator has the bandwidth to give it five minutes a day.
      </p>

      <CalloutBox type="danger">
        <p>The five most common Reviuzy misuses we see: auto-shipping templated replies, gating reviews by satisfaction (a Google policy violation), running contests without legal terms, flagging legitimate reviews as fake, and uploading stock photos of winners. Each one carries a soft-suppression risk on the GBP listing. The product guards against all five, but only when the operator follows the prompts.</p>
      </CalloutBox>

      <InlineCTA variant="book" text="Want a 60-minute walkthrough of the Reviuzy dashboard against your current GBP listing? Strategy doc sent regardless." />

      <SectionDivider />

      <KeyTakeaway
        points={[
          'Reviuzy is 100 dollars CAD per month, bundled in the AiLys Agency tier.',
          ...
          'Photos are uploaded by the client through the Reviuzy app. AiLys does not source winner photos.',
      ...
      <img
        src={meta.images.end}
        alt="Reviuzy add-on summary card with the four levers and the GBP wiring for a Quebec local business"
        className="w-full rounded-xl my-6"
        loading="lazy"
      />
    </article>
