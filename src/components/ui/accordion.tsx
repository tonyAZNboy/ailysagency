...
const AccordionTrigger = React.forwardRef<React.ElementRef<typeof AccordionPrimitive.Trigger>, React.ComponentPropsWithoutRef<typeof AccordionPrimitive.Trigger>>(({
  ...
  children,
  ...props
}, ref) => <AccordionPrimitive.Header className="flex text-xs text-justify">
    <AccordionPrimitive.Trigger ref={ref} className={cn("flex-1 font-medium transition-all hover:underline [&[data-state=open]>svg]:rotate-180 py-[2px] flex-row flex items-start justify-between text-sm text-right", className)} {...props}>
      {children}
